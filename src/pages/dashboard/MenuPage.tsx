...
const defaultCategories = ['Món chính', 'Ð? u?ng', 'Tráng mi?ng', 'Ãn v?t'];
...
export default function MenuPage() {
  ...
  const handleSaveItem = (e: React.FormEvent) => {
    ...
    if (menuItems.some(i => i.id === editingItem.id)) {
      ...
      toast.success('Ð? c?p nh?t món ãn');
    } else {
      setMenuItems(prev => [...prev, editingItem]);
      toast.success('Ð? thêm món m?i');
    }
    handleCloseModal();
  };

  const handleDeleteItem = (id: string) => {
    if (window.confirm('B?n có ch?c mu?n xóa món này?')) {
      setMenuItems(prev => prev.filter(i => i.id !== id));
      toast.success('Ð? xóa món ãn');
    }
  };
  ...
  const toggleStock = (id: string) => {
    setMenuItems(prev => prev.map(i => {
      if (i.id === id) {
        ...
        if (newStock) toast.success(`Ð? m? bán: ${i.name}`);
        else toast.warning(`Ð? t?m ngýng: ${i.name}`);
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto w-full animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tighter text-zinc-900 uppercase">Qu?n l? Th?c ðõn</h1>
          <p className="text-zinc-500 font-medium mt-1">C?u h?nh danh m?c, món ãn và tùy ch?n (Modifier).</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          <div key={item.id} className={`bg-white border-hard flex flex-col group ${!item.inStock ? 'opacity-60 grayscale' : ''}`}>
            ...
            <div className="p-4 flex flex-col flex-1">
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-dashed border-zinc-200">
                <span className="font-mono font-bold">{item.price.toLocaleString('vi-VN')}ð</span>
                <button 
        ...
        {filteredItems.length === 0 && (
          <div className="col-span-full py-20 flex flex-col items-center justify-center text-zinc-500 border-2 border-dashed border-zinc-200">
            ...
            <p className="font-medium">Chýa có món ãn nào.</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white border-hard shadow-hard w-full max-w-lg mb-20 animate-fadeIn relative">
            <div className="p-6 border-b border-hard flex justify-between items-center bg-zinc-50">
              <h3 className="text-xl font-bold uppercase tracking-tight">
                {menuItems.some(i => i.id === editingItem.id) ? 'S?a món ãn' : 'Thêm món ãn m?i'}
              </h3>
            ...
            <form onSubmit={handleSaveItem} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Giá bán (VNÐ)</label>
                  <input 
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Mô t?</label>
                <textarea 
                  rows={3}
